...
class CustomContractData extends Component {
  ...
  render() {
    ...
    if (Array.isArray(displayData)) {
      ...
      if (this.props.returnIndex !== undefined) {
        return(
          <span>{`${displayData[this.props.returnIndex]}`}{pendingSpinner}</span>
        )
      }
    ...
    if (typeof displayData === 'object') {
      if (this.props.returnKey !== undefined) {
        return(
          <span>{`${displayData[this.props.returnKey]}`}{pendingSpinner}</span>
        )
      }
...
CustomContractData.contextTypes = {
...
export default drizzleConnect(CustomContractData, mapStateToProps)
